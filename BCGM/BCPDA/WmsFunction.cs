    ...
    class WmsFunction
    {
        ...
        public static string GetMenuClass(string cKey)
        {
            using (var cmd = new SqlCommand(@"select cClass from BFunction WHERE cFunction=@cFunction"))
            {
                ...
                var wf = new WmsFunction(Properties.Settings.Default.PDACON);
        ...
        public static void GridFilter_Customizer()
        {
            ...
            rc.SetCustomizedString("DataErrorCellUpdateUnableToUpdateValue", "输入数据无效");
        }
        ...
        public static bool BoolCanOperate(string fCode,string fName)
        {
            ...
            using (var con = new SqlConnection(PDALogin.WmsConstring))
            {
                using (var cmd = new SqlCommand(sqlstr, con))
                {
                    cmd.Parameters.AddWithValue("@rCode", PDALogin.Lrole);
        ...
        /// <summary>
        /// 判断当前UGrid中的数据已经生成凭证的数据
        /// </summary>
        /// <param name="uGrid">要判断的Gird表</param>
        /// <param name="cAutoId">是采购单号，还是销售单号</param>
        /// <param name="cVouchType">哪种单据</param>
        public static void JAlreadyCreateVouch(UltraGrid uGrid, string cAutoId, string cVouchType)
        {
            using (var con = new SqlConnection { ConnectionString = PDALogin.WmsConstring })
            {
                using (var sqlcmd = new SqlCommand("select * from VouchHistory where cConCode=@cConCode and AutoID=@AutoID and VouchType=@VouchType"))
                {
                    sqlcmd.Connection = con;
                    con.Open();
                    foreach (var uRow in uGrid.Rows.GetAllNonGroupByRows())
                    {
                        sqlcmd.Parameters.Clear();
                        sqlcmd.Parameters.AddWithValue("@cConCode", uRow.Cells["cConCode"].Value);
                        sqlcmd.Parameters.AddWithValue("@AutoID", uRow.Cells[cAutoId].Value);
                        sqlcmd.Parameters.AddWithValue("@VouchType", cVouchType);
                        using (var dr = sqlcmd.ExecuteReader())
                        {
                            if (dr.Read())
                            {
                                uRow.Appearance.BackColor = Color.Red;
                            }
                            else
                            {
